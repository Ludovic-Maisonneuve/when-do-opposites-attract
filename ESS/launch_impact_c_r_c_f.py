...
for i_c_r, cr in enumerate(L_c):
    for i_c_f, cf in enumerate(L_c):

        i_MM = len(L_rho)

        print('cf', cf, flush=True)

        stop = False

        way = -1

        while stop == False and i_MM > 0:
        ...
        L_Result[i_c_r, i_c_f] = L_rho[i_MM]

np.save('results/ES_c_f_c_r', L_Result)
